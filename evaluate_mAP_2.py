...
def get_mAP(Yolo, dataset, score_threshold=0.25, iou_threshold=0.50, TEST_INPUT_SIZE=TEST_INPUT_SIZE):
    ...
    with open("mAP/results.txt", 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            print("tp", np.mean(tp), "fp", np.mean(fp), "tp+fn",gt_counter_per_class[class_name])
            print(text)
...
if __name__ == '__main__':       
    ...
    # map_dict = {}
    # iou_list = np.arange(0.2, 0.9, 0.1)
    # # iou_list = [0.5]
    # conf_list = np.arange(0.01, 0.3, 0.05)
    ...
    #         iou = iou_list[j]
    #         mAP = get_mAP(yolo, testset, score_threshold=conf, iou_threshold=iou, TEST_INPUT_SIZE=YOLO_INPUT_SIZE)
    #         map_list.append(mAP)
    #     map_dict[str(conf)] = map_list
    # print(map_dict)